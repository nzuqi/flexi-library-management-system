<?php
	...
	if (!$user->login_check($mysqli)){
		...
		/////////LIBRARY ACTIVITY////////////////////////////////////
		function countSActivity($activity,$year){
			$c=0;
			$con=mysqli_connect(HOST, USER, PASSWORD, DATABASE) or die(DB_CON_ERR);
			$q1="SELECT COUNT(UID) AS numrows FROM activity WHERE YEAR(uTimeS)=$year AND UActivity='".$activity."';";
		...
		if(isset($_GET['year'])){
			$aborrow=countSActivity('borrow',$_GET['year']);
			$areturn=countSActivity('return',$_GET['year']);
			$aread=countSActivity('read',$_GET['year']);
			$aother=countSActivity('other',$_GET['year']);
		}
		else{
			$aborrow=countSActivity('borrow',date('Y'));
			$areturn=countSActivity('return',date('Y'));
			$aread=countSActivity('read',date('Y'));
			$aother=countSActivity('other',date('Y'));
		}
		/////////////////////////////////////////////////////////////
		?>
		<div class="container">
			...
			<ul class="nav nav-tabs">
				...
				<li class="nav-item active"><a href="lib-analytics-yearly" class="nav-link"><strong><i class='glyphicon glyphicon-calendar'></i> YEARLY</strong></a></li>
			</ul>
			<!--HEADER-->
			<p>&nbsp;</p>
			<div class="well well-sm">
				<em><i class="glyphicon glyphicon-calendar"></i> Choose year:</em>&nbsp;
				<?php
				$q0="SELECT DISTINCT YEAR(UTimeS) FROM activity ORDER BY YEAR(UTimeS) Asc;";
				$res0=mysqli_query($mysqli,$q0);
				while ($rw0=mysqli_fetch_array($res0)){
					if(isset($_GET['year'])){
						if($_GET['year']==$rw0[0]){
							?><strong><?php echo $rw0[0]; ?></strong>&nbsp;&nbsp;<?php
						}
						else{
							?><a href="lib-analytics-yearly?year=<?php echo $rw0[0]; ?>"><?php echo $rw0[0]; ?></a>&nbsp;&nbsp;<?php
						}
					}
					else{
						?><a href="lib-analytics-yearly?year=<?php echo $rw0[0]; ?>"><?php echo $rw0[0]; ?></a>&nbsp;&nbsp;<?php
			...
			<div class="row">
				<div class="col-sm-4">
					...
					<?php
					if(isset($_GET['year']))
						$q="SELECT COUNT(IID) AS numrows FROM issue WHERE YEAR(iTimeS)=".$_GET['year'].";";
					else
						$q="SELECT COUNT(IID) AS numrows FROM issue WHERE YEAR(iTimeS)=YEAR(CURDATE());";
					...
					if($rw['numrows']>0){
						...
						<?php
						if(isset($_GET['year']))
							$q1="SELECT BID FROM issue WHERE YEAR(iTimeS)=".$_GET['year']." GROUP BY BID ORDER BY COUNT(*) DESC LIMIT 5;";
						else
							$q1="SELECT BID FROM issue WHERE YEAR(iTimeS)=YEAR(CURDATE()) GROUP BY BID ORDER BY COUNT(*) DESC LIMIT 5;";
			...
			<div class="row">
				<div class="col-sm-4">
					...
					<?php
					if(isset($_GET['year']))
						$q="SELECT COUNT(UID) AS numrows FROM activity WHERE YEAR(uTimeS)=".$_GET['year'].";";
					else
						$q="SELECT COUNT(UID) AS numrows FROM activity WHERE YEAR(uTimeS)=YEAR(CURDATE());";
					...
					if($rw['numrows']>0){
						...
						<?php
						if(isset($_GET['year']))
							$q1="SELECT SID,UActivity FROM activity WHERE YEAR(uTimeS)=".$_GET['year']." GROUP BY SID ORDER BY COUNT(*) DESC LIMIT 5;";
						else
							$q1="SELECT SID,UActivity FROM activity WHERE YEAR(uTimeS)=YEAR(CURDATE()) GROUP BY SID ORDER BY COUNT(*) DESC LIMIT 5;";
						$res1=mysqli_query($mysqli,$q1);
						while ($rw1=mysqli_fetch_array($res1)){
